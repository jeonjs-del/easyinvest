...
### 변동성 변형 듀얼모멘텀 데이터 소스 (2026-09-20 조사)
...
|---|---|---|---|---|
| KOSPI200 | `278530` (KODEX 200TR) | 실제 ETF(KRX) | 2017-11-21 | Yes — TR(총수익, 무분배 재투자)형이라 종가에 배당 반영 |
| KOSPI200 IT | `363580` (KODEX 200IT TR) | 실제 ETF(KRX) | 2020-09-25 | Yes — TR형 |
| SPY | `SPY` | 실제 ETF(Yahoo) | 1993-01-29 | Yes — `Adj Close` 사용(배당 재투자 반영) |
| 한국 국채 3년 | `114260` (KODEX 국고채3년) | 실제 ETF(KRX) | 2014-07-01 | 불명확 — `Close`만 사용, FDR엔 분배내역이 없어 분배금이 있었다면 반영 안 됐을 수 있음 |
| 한국 국채 10년 | `148070` (KIWOOM 국고채10년) | 실제 ETF(KRX) | 2014-07-01 | 위와 동일한 한계 |
| 한국 국채 30년 | `439870` (KODEX 국고채30년액티브) | 실제 ETF(KRX) | 2022-08-23 | 위와 동일한 한계 |
| 미국 단기국채 | `SHY` → 합성 컬럼 `US_BOND_SHORT_KRW` | 실제 ETF(Yahoo) + 환노출 합성 proxy | SHY 자체 2002-07-30, 합성 시리즈는 USD/KRW 시작일에 걸려 2003-12-01부터 유효 | Yes — SHY `Adj Close` 기준 |
| 미국 중기국채 | `IEF` → `US_BOND_MID_KRW` | 위와 동일 구조 | IEF 2002-07-30 / 합성 2003-12-01부터 | Yes |
| 미국 장기국채 | `TLT` → `US_BOND_LONG_KRW` | 위와 동일 구조 | TLT 2002-07-30 / 합성 2003-12-01부터 | Yes |
...
- **백테스트 실제 시작 시점**: 신호 계산은 2018-02(말), 첫 실현 월수익률은 2018-03부터
  나온다. 다른 자산(SPY 1993~, US채 2002~, USD/KRW 2003~)은 훨씬 이전부터 있지만,
  `strat_vol_dm`이 매 시점 KOSPI200(`278530`)과 SPY 3개월 모멘텀을 동시에 요구하므로
  **278530(KODEX 200TR) 상장일(2017-11-21)이 전체 시작 시점을 묶는 제약**이다.
- **상장 이전 구간 backfill**: 없음. 원본 KOSPI200 지수나 별도 프록시로 278530/363580
  상장 전 구간을 메우는 코드는 없다. 363580(200IT TR, 2020-09-25 상장) 이전에는 IT
  오버레이를 걸지 않고 KOSPI200 100%로 대체할 뿐, 가격 데이터 자체를 소급 생성하지
  않는다(`strat_vol_dm` 내 주석 참고). 439870(국고채30년, 2022-08-23 상장) 이전에는
  안전자산 후보에서 그냥 제외된다(`ret()`가 None → `_voldm_safe_asset`이 스킵).
